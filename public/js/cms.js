...
var triggerMediaUpload = function(elem)
{
    AWS.config.region = 'us-east-1'; // Region
    AWS.config.credentials = new AWS.CognitoIdentityCredentials({
        IdentityPoolId: 'us-east-1:92fd9d70-9bda-47af-a368-d02bec6c6e3b',
    });
    ...
    var filePart = fileName.substring(0, fileName.lastIndexOf("."));
    var suffixPart = fileName.substring(fileName.lastIndexOf(".") + 1, fileName.length) ;

    var key = 'media/' + suffixPart + '/' + filePart + "-" + uniqueId() + "." + suffixPart;
    var sibling = $(elem).siblings('input').first();
    $(sibling).val(key);

    var mediaType = mimeTypes.get("." + suffixPart);

    var bucket = new AWS.S3({params: {Bucket: 'classesmasses'}});
    var params = {Key: key, ContentType: mediaType, Body: file, ACL: 'public-read'};
    bucket.upload(params, function (err, data) {
        err ? alert('Upload failed.') : alert('Media uploaded.');
    });
}
